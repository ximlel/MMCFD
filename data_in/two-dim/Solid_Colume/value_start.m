...
for i=(shock+1):column
    ...
    if(norm(near_p)<R && norm(far_p)>R)
       CC(j,i) = quad2d(idx,j-1,j,i-1,i,'AbsTol',2e-4);
       CC(j,i) = max(CC(j,i),eps);
       CC(j,i) = min(CC(j,i),1.0-eps);
...
for j=1:line
fprintf(fid,'%16.13g\t',PHI(j,:));
fprintf(fid,'\n');
end
fclose(fid);

fid = fopen('Z_a.txt','wt');
for j=1:line
fprintf(fid,'%16.13g\t',CC(j,:));
fprintf(fid,'\n');
end
fclose(fid);
