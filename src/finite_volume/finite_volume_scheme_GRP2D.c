...
void finite_volume_scheme_GRP2D(struct flu_var * FV, const struct mesh_var * mv, const char * scheme, const char * problem) //R-K
{
	...
	for(l = 0; l < (int)config[5] && stop_step != 1; ++l) {
		...
		if (t_all >= plot_t) {
			for(i = 0; i < n_y-2; ++i)
				for(j = 0; j < n_x-2; ++j) {
					FV->RHO[i*n_x+j]=C.RHO_gC[i+1][j+1];
					FV->U[i*n_x+j]  =C.U_gC[i+1][j+1];
					FV->V[i*n_x+j]  =C.V_gC[i+1][j+1];
					FV->P[i*n_x+j]  =C.P_gC[i+1][j+1];
					FV->Z_a[i*n_x+j]=C.Z_sC[i+1][j+1];
				}
		...
		if (stop_step == 0) {
			...
			for(i = 0; i < n_y; ++i)
				for(j = 0; j < n_x; ++j) {
					...
					u_smid = C.U_sC[i][j] + SV.U_sx[i][j];
					area_L=0.5+u_smid*tau/dx;
					area_R=1.0-area_L;
					BN_C2U(C,U,i,j,0);
