...
void finite_volume_scheme_GRP2D(struct flu_var * FV, const struct mesh_var * mv, const char * scheme, const char * problem) //R-K
{
	...
	for(k=0, p=FX.ZRHO_F_gx; k<sizeof(struct flux_var)/sizeof(double **); k++,p++) {
		p = (double**)calloc(n_y+1,sizeof(double*));
		for(i=0; i<n_y+1; i++)
			p[i]=(double*)calloc(n_x+1,sizeof(double));
	}
		...
		if (stop_step == 0) {
			...
			for(i = 0; i < n_y; ++i)
				for(j = 0; j <=n_x; ++j) {
					...
					u_smid = U_R[i][j].u_s + U_R[ip1][j].u_s;
					v_smid = U_R[i][j].v_s + U_R[ip1][j].v_s;
					if (u_smid > 0.0)
						FX.stag_ZRHO_F_sx[i][j] = FX.stag_RHO_F_sx[i][j]*(z_sL - 0.25*tau*(u_smid*z_sxL+v_smid*z_syL));
					else
						FX.stag_ZRHO_F_sx[i][j] = FX.stag_RHO_F_sx[i][j]*(z_sR - 0.25*tau*(u_smid*z_sxR+v_smid*z_syR));
				}
			for(i = 0; i < n_y; ++i)
				for(j = 0; j < n_x; ++j) {
					ip1 = i<n_y-1?i+1:n_y-1;
			...
			for(j = 0; j <  n_x; ++j)
				for(i = 0; i <= n_y; ++i) {
					...
					GRP_RI_var_init(&GL, SV, C, dx, iL, j, 2);
					GRP_RI_var_init(&GR, SV, C, dx, iR, j, 3);
			...
			for(j = 0; j <  n_x; ++j)
				for(i = 0; i <= n_y; ++i) {
					...
					if (v_smid > 0.0)
						FX.stag_ZRHO_F_sy[i][j] = FX.stag_RHO_F_sy[i][j]*(z_sL - 0.25*tau*(u_smid*z_sxL+v_smid*z_syL));					
					else 
						FX.stag_ZRHO_F_sy[i][j] = FX.stag_RHO_F_sy[i][j]*(z_sR - 0.25*tau*(u_smid*z_sxR+v_smid*z_syR));
				}
		}
		for(j = 0; j < n_x; ++j)
			for(i = 0; i < n_y; ++i) {
